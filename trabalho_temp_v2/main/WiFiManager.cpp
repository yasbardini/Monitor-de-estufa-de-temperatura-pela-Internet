...
void WiFiManager::eventHandler( // função de callback para lidar com evento Wi-Fi
    void *arg,
...
void WiFiManager::begin() // função que inicia conexão Wi-Fi
{
    ESP_LOGI(TAG, "Inicializando Wi-Fi..."); 

    esp_err_t ret = nvs_flash_init(); 
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) 
    { // verifica se há erros de memória NVS que impedem a inicialização
        ESP_ERROR_CHECK(nvs_flash_erase());
        ESP_ERROR_CHECK(nvs_flash_init());
    }

    wifiEventGroup = xEventGroupCreate(); // cria um grupo de eventos Wi-Fi
