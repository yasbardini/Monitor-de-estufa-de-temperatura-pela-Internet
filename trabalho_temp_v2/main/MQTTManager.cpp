...
void MQTTManager::begin() // função que inicia a conexão com o broker 
{
...
void MQTTManager::mqttEventHandler( // função de callback para lidar com eventos MQTT
    void *handler_args,
...
void MQTTManager::handleData(const char *topic, int topicLen, const char *data, int dataLen) // função que processa os dados recebidos em um tópico 
{
    char topicBuffer[128] = {0}; // buffers locais
...
void MQTTManager::publishTemperature(const char *topic, float value) // publica temp convertendo valor
{
    char buffer[32];
...
void MQTTManager::publishLimit(const char *topic, float value) // função que publica limite de temperatura convertendo valor
{
    char buffer[32];
    std::snprintf(buffer, sizeof(buffer), "%.2f", value); 
    esp_mqtt_client_publish(client, topic, buffer, 0, 1, 0);
